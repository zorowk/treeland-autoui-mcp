...
def match_img(image, template_image, confidencevalue=0.7):
    """
        模板匹配图片
        :param image:大图
    ...
    """
    # 打开模板图片
    image.save(os.path.dirname(os.path.abspath(__file__)) + '/uos_public_operation/image/screen.png')
    print(os.path.abspath(__file__))
    image = cv.imread(os.path.dirname(os.path.abspath(__file__)) + '/uos_public_operation/image/screen.png')
    ...
    # 匹配度
    similarity = cv.minMaxLoc(result)[1] #获取最佳匹配位置
    # print(similarity)
    if similarity < confidencevalue:
        return None
    return {'result': (pos_x, pos_y),
            'rectangle': (pos_start[0], pos_start[1], template_image.shape[1], template_image.shape[0])}
